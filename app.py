...
def home():
    ...
    return render_template('home.html', docs=docs) # render the hone template
...
def home():
    ...
    """
    docs = db.exampleapp.find({}).sort("created_at", -1) # sort in descending order of created_at timestamp
    return render_template('moderator_home.html', docs=docs) # render the hone template
...
def add_spot_u():
    ...
    """
    spot_name = request.form['fspotname']
    address = request.form['faddress']
    location = request.form['flocation']
    type = request.form['ftype']
    purchase = request.form['fpurchase']
    noise = request.form['fnoise']
...
# route to accept form submission and create a new post
@app.route('/moderator/add', methods=['POST'])
def add_spot_m():
    """
    Route for POST requests to the create page.
    Accepts the form submission data for a new document and saves the document to the database.
    ...
    return redirect(url_for('moderator_home')) # tell the browser to make a request for the / route (the home function)
